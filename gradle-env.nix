...
#       }
{
      ...
      (
        ...
        in {
          ...
          buildNumber =
            if isNewer
            then dep.build
            else meta.buildNumber;
          lastUpdated =
            if isNewer
            then updated
            else meta.lastUpdated;
          versions =
            meta.versions
            or []
            ++ [
  ...
  mkInitScript = projectSpec: gradle: let
    ...
    inSettings = pred: script:
      optionalString pred (
        if versionAtLeast gradle.version "6.0"
        then ''
